-- OfficeBets v10. Data-preserving upgrade from v9 (with or without deletion patch).
begin;

alter table officebets.members add column if not exists avatar text not null default '🏎️' check(length(avatar) between 1 and 16);
alter table officebets.members add column if not exists bio text not null default '' check(length(bio)<=120);
alter table officebets.ledger add column if not exists market_id uuid;
create index if not exists ledger_market_history on officebets.ledger(market_id,id desc);
create index if not exists positions_market on officebets.positions(market_id);
create index if not exists markets_creator on officebets.markets(creator);
create index if not exists markets_category on officebets.markets(category);
create index if not exists ledger_user on officebets.ledger(user_id);
update officebets.categories set name='General' where id='general' and name='Supercharger Lounge'
 and not exists(select 1 from officebets.categories where id<>'general' and lower(name)='general');
create table if not exists officebets.price_history (
 id bigint generated always as identity primary key,
 market_id uuid not null references officebets.markets on delete cascade,
 at timestamptz not null default clock_timestamp(), odds numeric[] not null, kind text not null
);
create index if not exists history_market on officebets.price_history(market_id,id);
create table if not exists officebets.comments (
 id bigint generated always as identity primary key, market_id uuid not null references officebets.markets on delete cascade,
 user_id uuid not null references officebets.members, at timestamptz not null default clock_timestamp(),
 body text not null check(length(btrim(body)) between 1 and 1000)
);
create index if not exists comments_market on officebets.comments(market_id,id desc);
create index if not exists comments_user on officebets.comments(user_id);
...
create or replace function officebets.log_entry(u uuid,k text,a numeric,d text) returns void language sql set search_path='' as $$
...
$$;
create or replace function officebets.record_price() returns trigger language plpgsql set search_path='' as $$
...
drop trigger if exists record_price on officebets.markets;
...
-- Existing prices are a starting snapshot, never fabricated historical trades.
insert into officebets.price_history(market_id,odds,kind)
 select m.id,array(select case when m.winner is not null then case when i-1=m.winner then 1 else 0 end
 else (1/m.reserves[i])/(select sum(1/v) from unnest(m.reserves) v) end from generate_subscripts(m.reserves,1) i order by i),'UPGRADE'
 from officebets.markets m where not exists(select 1 from officebets.price_history h where h.market_id=m.id);
-- Only a revision counter is exposed to Realtime. Team data stays behind the validated RPCs.
create table if not exists public.officebets_updates (id integer primary key check(id=1), revision bigint not null);
insert into public.officebets_updates values(1,0) on conflict do nothing;
alter table public.officebets_updates enable row level security;
revoke all on public.officebets_updates from public,anon,authenticated;
grant select on public.officebets_updates to anon,authenticated;
drop policy if exists team_revision_read on public.officebets_updates;
create policy team_revision_read on public.officebets_updates for select to anon,authenticated using(true);
create or replace function officebets.publish_revision() returns trigger language plpgsql set search_path='' as $$
begin update public.officebets_updates set revision=new.value where id=1; return new; end $$;
drop trigger if exists publish_revision on officebets.revision;
...
do $$ begin
 ...
 not exists(select 1 from pg_publication_tables where pubname='supabase_realtime' and schemaname='public' and tablename='officebets_updates') then
 alter publication supabase_realtime add table public.officebets_updates;
 end if;
end $$;
...
begin
 ...
 'markets',(select coalesce(jsonb_agg(jsonb_build_object('id',m.id,'creatorId',m.creator,'title',m.title,'description',m.description,
   ...
   'everTraded',exists(select 1 from officebets.positions p where p.market_id=m.id),
 ...
 'transactions',(select coalesce(jsonb_agg(jsonb_build_object('id',l.id,'timestamp',l.at,'userId',l.user_id,'userName',coalesce(u.name,'House'),
...
begin
 ...
 if p_action='boost' then
   ...
   if p_action='edit_market' then
     ...
     if m.winner is not null or exists(select 1 from officebets.positions where market_id=m.id) then raise exception 'Editing is locked after the first trade or resolution'; end if;
   end if;
   ...
   if not actor.is_admin and (uid<>m.creator or m.winner is not null or exists(select 1 from officebets.positions where market_id=m.id)) then
   ...
   if p_action='settle' then
     ...
     update officebets.markets set reserves=r,vault=m.vault where id=m.id;
   end if;
...
create or replace function public.ob_market_view(p_market uuid) returns jsonb language sql security definer set search_path='' as $$
 ...
 'activity',coalesce((select jsonb_agg(x.item order by x.at desc,x.id desc) from (
   (select l.at,l.id,jsonb_build_object('id','t'||l.id,'at',l.at,'userId',l.user_id,'name',coalesce(u.name,'House'),'avatar',coalesce(u.avatar,'⚡'),'kind',l.kind,'amount',l.amount,'text',l.details) item
...
grant execute on function public.ob_market_view(uuid) to anon,authenticated;
