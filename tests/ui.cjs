...
(async()=>{
 ...
 await page.locator('#predictionTools').getByRole('button',{name:'Resolve early',exact:true}).click();assert.equal(await page.locator('#settleModal').isVisible(),true);assert.equal(await page.locator('#tradeModal').isVisible(),false);await page.evaluate(()=>closeSettleModal());
 ...
 await page.screenshot({path:'/tmp/officebets-desktop.png',fullPage:true});
 for(const width of [390,760,1024]){await page.setViewportSize({width,height:900});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),`Page overflows at ${width}px`);assert.equal(await page.locator('#sidebarNewPrediction').isVisible(),true);assert.equal(await page.locator('#endingSoon').isVisible(),true);if(width===390)await page.screenshot({path:'/tmp/officebets-mobile.png',fullPage:true});}
